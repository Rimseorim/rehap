...
| 데드리프트 | d. 굴곡·신전 모두 통증 (복합형, 코어+가동성) | 전굴·후굴 유발 테스트 ("둘 다 아파요" 선택) | 신규 추가 |
...
- **deadlift/허리 전굴·후굴 유발 테스트(test-flexion)**: "둘 다 아파요" 3번째 선택지(`extra_choice`) 추가, 신규 **cause-d(복합형)**로 라우팅.
  cause-d 재활루트는 cause-a/b 운동 중 공통·핵심 운동(복압 브레이싱, Couch Stretch, 데드 버그, 버드 독, RDL-중립척추)을 결합해 구성.
  사유: 굴곡·신전 양쪽 모두 아픈 경우 기존 cause-a(굴곡성) 또는 cause-b(신전성) 어느 쪽도 증상 설명이 맞지 않아, 기존 `note`("두 방향 모두 아프면 의사 진료 권장")만 있고 실제 선택 경로가 없던 사각지대였음.
  `test()` 렌더 함수에 범용 `extra_choice` 3번째 버튼 지원을 추가(향후 유사 케이스에 재사용 가능).

**허리 결론**: 23개 원인 전부 일치 (보류 2건 + 신규 1건 모두 처리 완료).
...
**팔꿈치 결론**: 5개 동작 × 3원인 = 15개 전부 일치. 보류 없음.
